...
use mosekmodel::{SolutionType,Model,Sense,equal_to,ModelItemIndex, Variable, matrix, in_psd_cone,vstack};
use mosekmodel::expr::*;
...
fn hankel(n : usize, i_ : isize, a : f64) -> matrix::SparseMatrix {
    if i_ < 0 || i_ as usize > 2*n {
        ...
        if i < n + 1 {
            matrix::sparse(n+1, n+1, 
                           (0..i+1).rev().collect::<Vec<usize>>().as_slice(),
                           (0..i+1).collect::<Vec<usize>>().as_slice(),
                           vec![a; i+1].as_slice())
        } 
        else {
            matrix::sparse(n+1, n+1, 
                           (i-n..n+1).rev().collect::<Vec<usize>>().as_slice(),
                           (i-n..n+1).collect::<Vec<usize>>().as_slice(),
...
fn diff(model : & mut Model, x : & Variable<1>) -> Variable<1> {
    ...
    _ = model.constraint(None,
                         &u.clone().reshape(&[n,1]).sub(x.clone().slice(&[1],&[n+1]).reshape(&[n,1]).mul_elm(matrix::dense(n,1,(1..n+1).map(|v| v as f64).collect::<Vec<f64>>().as_slice()))), 
                         equal_to(vec![0.0;n].as_slice()).with_shape(&[n,1]));
    u
}

fn fitpoly(data : & matrix::DenseMatrix, n : usize) -> Vec<f64> {
    ...
    _ = model.constraint(None,
                         &ub.clone().sub(vstack![z.clone().sub(dx.clone().slice(&[0],&[1])), dx.clone().slice(&[1],&[n])]),
                         equal_to(vec![0.0; n]));
    ...
    _ = model.constraint(None,
                         &lb.clone().sub(vstack![z.clone().add(dx.clone().slice(&[0],&[1])), dx.clone().slice(&[1],&[n])]),
                         equal_to(vec![0.0; n]));
